...
/// 啟動 joiner 端 tunnel：
/// 1. 連接 /tunnel WS
/// 2. bind TCP listener 在 127.0.0.2:6112
/// 3. 等待 War3 TCP 連入
/// 4. bridge TCP ↔ WS
pub async fn run_joiner_tunnel(
    ...
    info!(%token_short, "Joiner tunnel: 連接 WS");
    ...
    // Bind TCP listener on 127.0.0.2:6112
    ...
    let tcp_stream = match tcp_listener.accept().await {
        ...
        Err(e) => {
            ...
            let _ = event_tx.send(TunnelEvent::Finished {
                error: Some(format!("TCP accept 失敗: {e}")),
            });
            return;
        }
    };

    // Drop listener，不再接受新連線
    drop(tcp_listener);
...
/// 啟動 host 端 tunnel：
/// 1. 連接 /tunnel WS
/// 2. TCP 連到 127.0.0.1:6112（War3 的 listening socket）
/// 3. bridge TCP ↔ WS
pub async fn run_host_tunnel(
    ...
    info!(%token_short, "Host tunnel: 連接 WS");
    ...
    // TCP 連到本地 War3
    let tcp_stream = match TcpStream::connect(SocketAddr::V4(HOST_WAR3_ADDR)).await {
        Ok(s) => s,
        Err(e) => {
            error!(%token_short, %e, "無法連接 War3 (127.0.0.1:6112)");
            let _ = event_tx.send(TunnelEvent::Finished {
                error: Some(format!("無法連接 War3: {e}")),
            });
            return;
        }
    };
